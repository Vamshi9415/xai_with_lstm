...
class SpamDetectionPipeline:
    ...
    def debug_classification_report(self, model_name: str = None):
        """Debug method to inspect classification report structure."""
        if not self.results:
        ...
        if model_name and model_name in self.results:
            ...
            models_to_check = list(self.results.keys())[:1]  # Check first model as example
        
        for model in models_to_check:
            print(f"\n=== Debug info for {model} ===")
            class_report = self.results[model]['classification_report']
            print("Classification report keys:", list(class_report.keys()))
...
#    Save pipeline
    ...
    print("Pipeline ready! Uncomment the lines above to run the full analysis.")e